...
/**
 * Class TZ_Addons_Waler_Mega_Menu_Edit
 *
 * Class for adding more controllers into a menu item
 */
class TZ_Addons_Waler_Mega_Menu_Edit extends Walker_Nav_Menu_Edit {
	/**
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$mega = tz_addons_recurse_parse_args( $mega, TZ_Addons_Mega_Menu::default_settings() );
		...
		if ( $cancel ) {
			...
			$link->nodeValue = esc_html__( 'Settings', 'tz-addons' );
 ...
 */
class TZ_Addons_Mega_Menu_Walker extends Walker_Nav_Menu {
	/**
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_mega = tz_addons_recurse_parse_args( $item_mega, TZ_Addons_Mega_Menu::default_settings() );
...
class TZ_Addons_Mega_Menu {
	...
	public function __construct() {
		$this->modals = apply_filters( 'tz_addons_mega_menu_modal_panels', array(
			'menu',
			'title',
			'mega',
			'design',
		...
		add_action( 'wp_ajax_tz_addons_save_menu_item_data', array( $this, 'save_menu_item_data' ) );
	}
	...
	public function edit_walker() {
		return 'TZ_Addons_Waler_Mega_Menu_Edit';
	}
	...
	public function scripts( $hook ) {
		...
		wp_register_style( 'font-awesome', TZ_TF_CSS_URL . 'font-awesome.min.css' );
		wp_register_style( 'tz-addons-mega-menu-admin', TZ_TF_CSS_URL . 'admin/mega-menu.css', array(
			'media-views',
			'wp-color-picker',
			'font-awesome',
		) );
		wp_enqueue_style( 'tz-addons-mega-menu-admin' );

		wp_register_script( 'tz-addons-mega-menu-admin', TZ_TF_JS_URL . 'admin/mega-menu.js', array(
			'jquery',
			'jquery-ui-resizable',
			'wp-util',
			'wp-color-picker',
		...
		wp_enqueue_media();
		wp_enqueue_script( 'tz-addons-mega-menu-admin' );

		wp_localize_script( 'tz-addons-mega-menu-admin', 'tzMenuModal', $this->modals );
	}
	...
	public function modal() {
		?>
		<div id="kmm-mega-menu" tabindex="0" class="kmm-mega-menu">
			<div class="kmm-modal media-modal wp-core-ui">
				<button type="button" class="button-link media-modal-close kmm-modal-close">
					<span class="media-modal-icon"><span class="screen-reader-text"><?php esc_html_e( 'Close', 'tz-addons' ) ?></span></span>
				</button>
				<div class="media-modal-content">
					<div class="kmm-frame-menu media-frame-menu">
						<div class="kmm-menu media-menu"></div>
					</div>
					<div class="kmm-frame-title media-frame-title"></div>
					<div class="kmm-frame-content media-frame-content">
						<div class="kmm-content"></div>
					</div>
					<div class="kmm-frame-toolbar media-frame-toolbar">
						<div class="kmm-toolbar media-toolbar">
							<div class="kmm-toolbar-primary media-toolbar-primary search-form">
								<button type="button" class="button kmm-button-save media-button button-primary button-large"><?php esc_html_e( 'Save Changes', 'tz-addons' ) ?></button>
								<button type="button" class="button kmm-button-cancel media-button button-secondary button-large"><?php esc_html_e( 'Cancel', 'tz-addons' ) ?></button>
			...
			<div class="media-modal-backdrop kmm-modal-backdrop"></div>
		</div>
		<?php
	}
	...
	public function templates() {
		foreach ( $this->modals as $template ) {
			$file = TZ_TF_INC_PATH . 'mega-menu/templates/menu-' . $template . '.php';

			if ( ! file_exists( $file ) ) {
				continue;
			}
			?>
			<script type="text/html" id="tmpl-kmm-<?php echo esc_attr( $template ) ?>">
				<?php include( $file ); ?>
			</script>
			<?php
		}
	...
	public function save_menu_item_data() {
		...
		foreach ( $data['menu-item-mega'] as $id => $meta ) {
			...
			$old_meta = tz_addons_recurse_parse_args( $old_meta, TZ_Addons_Mega_Menu::default_settings() );
			$meta     = tz_addons_recurse_parse_args( $meta, $old_meta );
